...
func checkUser(username, password string) (bool, error) {
	...
	var utype User // if user in the db, then find this user password
	...
	return false, nil // false indicate if the user is in db, nil indicate operation run into probs
}

func addUser(user *User) (bool, error) {
	query := elastic.NewTermQuery("username", user.Username)
	searchResult, err := readFromES(query, USER_INDEX) // see if same username is already in db
	if err != nil {
		return false, err
	}
...
func handlerLogin(w http.ResponseWriter, r *http.Request) { // responsewriter is interface, so no pointer
	...
	token := jwt.NewWithClaims(jwt.SigningMethodHS256, jwt.MapClaims{
		...
		"exp":      time.Now().Add(time.Hour * 24).Unix(), //token effective time
	})
