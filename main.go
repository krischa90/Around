...
func handlerPost(w http.ResponseWriter, r *http.Request) {
	...
	// save image to GCS
	file, header, err := r.FormFile("image") // file: content; header: file property
	if err != nil {
	...
	id := uuid.New()                      // produce an unique id
	mediaLink, err := saveToGCS(file, id) // must be unique name
	if err != nil {
	...
	if p.Type == "image" {
		uri := fmt.Sprintf("gs://%s/%s", BUCKET_NAME, id) // Sprintf save printf to uri as a string
...
func saveToES(i interface{}, index string, id string) error {
	...
	_, err = client.Index(). // prepare statement=
					Index(index). // set id
					Id(id).
					BodyJson(i).
					Do(context.Background())
...
func handlerCluster(w http.ResponseWriter, r *http.Request) {
	...
	query := elastic.NewRangeQuery(term).Gte(0.9) // greater than 0.9 score
